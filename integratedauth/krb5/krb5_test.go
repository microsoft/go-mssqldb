...
func TestReadKrb5ConfigHappyPath(t *testing.T) {
	...
	if actual.DNSLookupKDC == nil || *actual.DNSLookupKDC != false {
		t.Errorf("Expected DNSLookupKDC %v, found %v", false, actual.DNSLookupKDC)
	}

	if actual.UDPPreferenceLimit == nil || *actual.UDPPreferenceLimit != 1234 {
...
func TestNewKrb5ConfigFromReader_DNSLookupKDC(t *testing.T) {
	krb5ConfStringLookupKDCTrue := `
[libdefaults]
    dns_lookup_kdc = true`

	krb5ConfStringLookupKDCFalse := `
[libdefaults]
	...
	tests := []struct {
		...
		krb5Conf             string
		input                *krb5Login
		expectedDNSLookupKDC bool
	}{
		{
			name:     "DNSLookupKDC left as default when not specified, true",
			krb5Conf: krb5ConfStringLookupKDCTrue,
			input: &krb5Login{
				DNSLookupKDC: nil,
			},
			expectedDNSLookupKDC: true,
		},
		{
			name:     "DNSLookupKDC left as default when not specified, false",
			krb5Conf: krb5ConfStringLookupKDCFalse,
			input: &krb5Login{
				DNSLookupKDC: nil,
			},
			expectedDNSLookupKDC: false,
		},
		{
			name:     "DNSLookupKDC overridden from default (false) when specified",
			krb5Conf: krb5ConfStringLookupKDCFalse,
			input: &krb5Login{
				DNSLookupKDC: &trueValue,
			},
			expectedDNSLookupKDC: trueValue,
		},
		{
			name:     "DNSLookupKDC overridden from default (true) when specified",
			krb5Conf: krb5ConfStringLookupKDCTrue,
			input: &krb5Login{
				DNSLookupKDC: &falseValue,
			},
			expectedDNSLookupKDC: falseValue,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.input.loginMethod = none
			reader := strings.NewReader(tt.krb5Conf)
			config, err := newKrb5ConfigFromReader(reader, tt.input)
			...
			if config != nil && config.LibDefaults.DNSLookupKDC != tt.expectedDNSLookupKDC {
				t.Errorf("DNSLookupKDC Expected %v, found %v", config.LibDefaults.DNSLookupKDC, tt.expectedDNSLookupKDC)
			}
		})
	}
}

func TestNewKrb5ConfigFromReader_UDPPreferenceLimit(t *testing.T) {
	krb5ConfStringUDPPreferenceLimit := `
[libdefaults]
	...
	tests := []struct {
		...
		krb5Conf                   string
		input                      *krb5Login
		expectedUDPPreferenceLimit int
	}{
		{
			name:     "UDPPreferenceLimit left as default when not specified, true",
			krb5Conf: krb5ConfStringUDPPreferenceLimit,
			input: &krb5Login{
				UDPPreferenceLimit: nil,
			},
			expectedUDPPreferenceLimit: 1234,
		},
		{
			name:     "UDPPreferenceLimit overridden from default (1234) when specified",
			krb5Conf: krb5ConfStringUDPPreferenceLimit,
			input: &krb5Login{
				UDPPreferenceLimit: &udpLimitPref,
			},
			expectedUDPPreferenceLimit: udpLimitPref,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if config != nil && config.LibDefaults.UDPPreferenceLimit != tt.expectedUDPPreferenceLimit {
				t.Errorf("UDPPreferenceLimit Expected %v, found %v", config.LibDefaults.UDPPreferenceLimit, tt.expectedUDPPreferenceLimit)
			}
		})
	}
}
...
func TestGetAuth(t *testing.T) {
	...
	if actual.krb5Config.DNSLookupKDC == nil || *actual.krb5Config.DNSLookupKDC != false {
		t.Errorf("Expected DNSLookupKDC %v, found %v", false, actual.krb5Config.DNSLookupKDC)
	}

	if actual.krb5Config.UDPPreferenceLimit == nil || *actual.krb5Config.UDPPreferenceLimit != 1234 {
