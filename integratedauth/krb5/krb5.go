...
// The set of supported connection string parameters
const (
...
// The set of static error messages which can be raised from this package. Others come from fmt.Errorf.
var (
...
func fileExistsOS(filename string, errWhenFileNotFound error) (bool, error) {
	...
	return false, fmt.Errorf("fileExistsOS : unexpected error %v : %w", errWhenFileNotFound, err)
}
...
func (k *krbAuth) NextBytes(bytes []byte) ([]byte, error) {
	...
	case gssapi.StatusContinueNeeded:
		return nil, nil
	default:
		return nil, fmt.Errorf("unexpected gssapi status: %+v", status)
	}
}
...
func newKrb5ConfigFromFile(krb5Login *krb5Login) (*config.Config, error) {
	...
	return newKrb5ConfigFromReader(f, krb5Login)
}

func newKrb5ConfigFromReader(reader io.Reader, krb5Login *krb5Login) (*config.Config, error) {
	cfg, err := config.NewFromReader(reader)
	if err != nil {
		return nil, err
	}

	if krb5Login.DNSLookupKDC != nil {
		cfg.LibDefaults.DNSLookupKDC = *krb5Login.DNSLookupKDC
	}

	if krb5Login.UDPPreferenceLimit != nil {
