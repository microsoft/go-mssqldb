...
* Add native JSON type support for SQL Server 2025 Preview. The driver now negotiates JSON support during TDS login and properly handles the new JSON data type (0xF5). JSON data is returned as `[]byte` and can be scanned into `[]byte`, `string`, or types implementing `sql.Scanner`.
