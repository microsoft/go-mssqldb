...
func compareValue(a interface{}, expected interface{}) bool {
	...
	case Time:
		...
		return false
	case DateTimeOffset:
		// compare DateTimeOffset to time.Time returned by SQL
		if got, ok := a.(time.Time); ok {
			return got.Equal(time.Time(expected))
		}
		return false
	case NullDate:
...
func setupTable(ctx context.Context, t *testing.T, conn *sql.Conn, tableName string) (err error) {
	...
	[test_mssqldate] [date] NULL,
	[test_mssqldate_2] [date] NULL,
	[test_date] [date] NULL,
